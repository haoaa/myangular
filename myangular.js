    ...
    this.$get = ['$injector', '$rootScope', '$parse', '$controller', '$http', '$interpolate', function(
        ...
        function compile($compileNodes, maxPriority) {
            ...
            return function publicLinkFn(scope, cloneAttachFn, options) {
                ...
                var $linkNode;
                if (cloneAttachFn) {
                    $linkNode = $compileNodes.clone();
                    cloneAttachFn($linkNode, scope);
                } else {
                    $linkNode = $compileNodes;
                }
                _.forEach(transcludeControllers, function(controller, name) {
                    $linkNode.data('$' + name + 'Controller', controller.instance);
                });
                ...
                _.forEach(linkFns, function(linkFn) {
                    ...
                    if (linkFn.nodeLinkFn) {
                        ...
                        if (linkFn.nodeLinkFn.transcludeOnThisElement) {
                            boundTranscludeFn = function(transcludeScope, cloneAttachFn, transcludeControllers, containingScope) {
                                if (!transcludeScope) {
        ...
        function applyDirectivesToNode(
            ...
            function nodeLinkFn(childLinkFn, scope, linkNode, boundTranscludeFn) {
                ...
                function scopeBoundTranscludeFn(transcludeScope, cloneAttachFn) {
                    var transcludeControllers;
                    if (!transcludeScope || !transcludeScope.$watch ||
                        !transcludeScope.$evalAsync) {
                        cloneAttachFn = transcludeScope;
                        transcludeScope = undefined;
                    }
                    if (hasElementTranscludeDirective) {
                        transcludeControllers = controllers;
                    }
                    return boundTranscludeFn(transcludeScope, cloneAttachFn, transcludeControllers, scope);
                }
        ...
        function compileTemplateUrl(directives, $compileNode, attrs, previousCompileContext) {
            ...
            origAsyncDirective.transclude = null;
            $compileNode.empty();

            $http.get(url).success(function(template) {
                $compileNode.html(denormalizeTemplate(template));
...
},{"../src/hash_map":12,"lodash":2}],16:[function(require,module,exports){
'use strict';
    ...
    while (this.index < this.text.length) {
        ...
        if (ch === '.' || this.isNumber(ch)) {
            ...
            if (ch === 'e' && this.isExpOperator(nextCh)) {
                number += ch;
            } else if (this.isExpOperator(ch) && prevCh === 'e' && nextCh &&
...
AST.prototype.ast = function(text) {
    ...
    // console.debug('tokens: ', JSON.stringify(this.tokens));
...
AST.prototype.constant = function() {
    ...
    if (this.peek().value === undefined) throw 'Invalid constant: ' +
    this.peek().text;
...
AST.prototype.peek = function(e1, e2, e3, e4) {
    if (this.tokens.length > 0) {
        ...
        if (e1 === text || e2 === text || e3 === text || e4 === text ||
            (!e1 && !e2 && !e3 && !e4)) {
...
function markConstantAndWatchExpressions(ast, $filter) {
    ...
    case AST.CallExpression:
        ...
        allConstant = stateless ? true : false;
        argsToWatch = [];
        _.each(ast.arguments, function(argument) {
            markConstantAndWatchExpressions(argument, $filter);
            allConstant = allConstant && argument.constant;
...
ASTCompiler.prototype.compile = function(text) {
    ...
    if (assignable) {
        ...
        extra = 'fn.assign =function(s,v,l){' +
            (this.state.assign.vars.length ? 'var ' +
                    this.state.assign.vars.join(',') + ';' : ''
...
ASTCompiler.prototype.recurse = function(ast, context, create) {
    ...
    case AST.CallExpression:
        ...
        if (ast.filter) {
            ...
            args = _.map(ast.arguments, _.bind(function(argument) {
                return 'ensureSafeObject(' + this.recurse(argument) + ')';
            }, this));
            ...
            if (callContext.name) {
                ...
                if (callContext.computed) {
                    callee = this.computedMember(callContext.context,
                        callContext.name);
                } else {
                    callee = this.nonComputedMember(callContext.context,
                        callContext.name);
                }
            }
            this.addEnsureSafeFunction(callee);
            return callee + '&& ensureSafeObject(' + callee + '(' +
                    args.join(',') + '))';
        }
    case AST.AssignmentExpression:
        var leftContext = {};
        ...
        if (leftContext.computed) {
            leftExpr = this.computedMember(leftContext.context,
                leftContext.name);
        } else {
            leftExpr = this.nonComputedMember(leftContext.context,
                leftContext.name);
        }
...
ASTCompiler.prototype.watchFns = function() {
    var result = [];
    _.each(this.state.inputs, _.bind(function(inputName) {
        var fnString = 'var ' + inputName + ' =function(s){' +
            (this.state[inputName].vars.length > 0 ? 'var ' +
                this.state[inputName].vars.join(',') + ';' : ''
...
ASTCompiler.prototype.filterPrefix = function() {
    if (_.isEmpty(this.state.filters)) {
        ...
        var parts = _.map(this.state.filters,
            _.bind(function(varName, filterName) {
...
function inputsWatchDelegate(scope, listenerFn, valueEq, watchFn) {
    ...
    return scope.$watch(function() {
        var changed = false;
        _.each(inputExpressions, function(inputExpr, i) {
